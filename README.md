...
这是当前唯一推荐的桌面启动入口。仓库里保留的其他桌面模块仅用于兼容旧导入路径，不再承载实际界面实现。
...
1. 先启动桌面应用。
2. 先到“数据导入”页确认或自动检测 `RimWorld Data` 目录，并导入原版数据。
3. 到“人物创建”页建立攻击方和防守方模板。
4. 到“场景设计”页选择攻防人物，设置距离和命中率，保存测试场景。
5. 到“结果对比”页把一个或多个已保存场景加入对比表，并按列排序观察输出与承伤差异。

人物、场景和对比结果会自动保存在系统用户数据目录中。Windows 默认位置为 `%APPDATA%\RimDataAnalysis`；如需指定位置，可设置环境变量 `RIM_DATA_ANALYSIS_APP_STATE_DIR`。
